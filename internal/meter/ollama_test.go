...
// ollamaGenerateStream is a real NDJSON stream from Ollama's native
// /api/generate endpoint. Each chunk carries the "response" field rather
// than message.content.
const ollamaGenerateStream = "{\"model\":\"llama3.2\",\"created_at\":\"2024-01-01T00:00:00Z\",\"response\":\"Hello\",\"done\":false}\n{\"model\":\"llama3.2\",\"created_at\":\"2024-01-01T00:00:00Z\",\"response\":\" from\",\"done\":false}\n{\"model\":\"llama3.2\",\"created_at\":\"2024-01-01T00:00:00Z\",\"response\":\" generate\",\"done\":true,\"total_duration\":12345,\"prompt_eval_count\":15,\"eval_count\":3}\n"

func TestStreamMeterOllamaGenerate(t *testing.T) {
	var dst bytes.Buffer
	m := NewStreamMeter(Ollama)
	if err := m.Tee(&dst, strings.NewReader(ollamaGenerateStream)); err != nil {
		t.Fatalf("Tee error: %v", err)
	}
	if dst.String() != ollamaGenerateStream {
		t.Fatalf("streamed bytes were altered: got %q want %q", dst.String(), ollamaGenerateStream)
	}
	res := m.Result()
	if !res.HasUsage {
		t.Fatalf("HasUsage = false, want true")
	...
	want := policy.Usage{
		Model: "llama3.2", InputTokens: 15, OutputTokens: 3,
	}
	if res.Usage != want {
		t.Fatalf("Usage = %+v, want %+v", res.Usage, want)
	}
